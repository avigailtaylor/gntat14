...
if [ "$1" = "--help" ]; then
echo -e "--------------------------------\n\nDESCRIPTION\n\nSeed connectivity analysis using seed randomisation. Shell script to run standalone executable file.\n\n\nUSAGE\n\nTo run the shell script type:\n\n./run_sr_seed_exe.sh <mcr_directory> <argument_list>\n\nat Linux command prompt.\n\n<mcr_directory> is the directory where version 8.2 of MCR is installed or the directory where MATLAB is installed on the machine. <argument_list> is the following list of arguments:\n\nntwk_edgelist_file ntwk_header seed_file s_header rands parallelise quickview_flag basic_results_file [ --results_dir results_dir --results_tag results_tag ] [ --node_attribute_file node_attribute_file --node_attribute_indices node_attribute_indices [ --node_attribute_bins node_attribute_bins ] ]\n"

echo -e "\nINPUT\n"
echo -e "               ntwk_edgelist_file = Full or relative path to text file containing the network of interest, represented as a list
 of edges (Two columns, tab/ space-delimited).\n" 
echo -e "               ntwk_header = Boolean (true, false, 1 or 0) flag to indicate whether or not to remove a header-line in ntwk_edgel
ist_file.\n" 
...
echo -e "               rands = Number of randomisations to perform.\n"
echo -e "               parallelise = Boolean (true, false, 1 or 0) flag to indicate whether or not to use multiple processors for parall
el permutations.\n"
echo -e "               quickview_flag = Boolean (true, false, 1 or 0) flag to indicate whether or not to plot a 'quickview' of the direc
t network among seeds.\n"
