...
echo -e "--------------------------------\n\nDESCRIPTION\n\nSeed to backbone connectivity analysis using network permutation. Shell script to run standalone executable file.\n\n\nUSAGE\n\nTo run the shell script type:\n\n./run_np_seed2back_exe.sh <mcr_directory> <argument_list>\n\nat Linux command prompt.\n\n<mcr_directory> is the directory where version 8.2 of MCR is installed or the directory where MATLAB is installed on the machine. <argument_list> is the following list of arguments:\n\nntwk_edgelist_file ntwk_header seed_file s_header bb_file bb_header s_bb_comb perms num_switches protect_perm_step parallelise quickview_flag basic_results_file [ --results_dir results_dir --results_tag results_tag ]\n"
...
echo "          results_dir/results_tag.SEED_BB_SCORES (List of empirical P-values for each seed's and backbone-node's connectedness to other seeds an
d backbone-nodes in the direct network among seeds and backbone-nodes (P-values only given for seeds and backbone-nodes participating in 
the direct network among seeds and backbone-nodes).)"
...
if [ "x$1" = "x" ]; then
  ...
  eval "\"${exe_dir}/np_seed2back_exe\"" $args
fi
exit
