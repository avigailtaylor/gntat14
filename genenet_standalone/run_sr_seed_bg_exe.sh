...
echo -e "--------------------------------\n\nDESCRIPTION\n\nSeed connectivity analysis using seed randomisation, and restricting the analysis to a specified sub-network (background) of the primary network. Shell script to run standalone executable file.\n\n\nUSAGE\n\nTo run the shell script type:\n\n./run_sr_seed_bg_exe.sh <mcr_directory> <argument_list>\n\nat Linux command prompt.\n\n<mcr_directory> is the directory where version 8.2 of MCR is installed or the directory where MATLAB is installed on the machine. <argument_list> is the following list of arguments:\n\nntwk_edgelist_file ntwk_header bg_file bg_header seed_file s_header rands parallelise quickview_flag basic_results_file [ --results_dir results_dir --results_tag results_tag ] [ --node_attribute_file node_attribute_file --node_attribute_indices node_attribute_indices [ --node_attribute_bins node_attribute_bins ] ]\n"
...
echo "          results_dir/results_tag.SEEDS_PRESENT (List of seeds present in the background network of interest.)"
...
if [ "x$1" = "x" ]; then
  ...
  eval "\"${exe_dir}/sr_seed_bg_exe\"" $args
fi
exit
