...
%Version: 1
%Date: 20140722
%Description: Called by np_seed2back_aux. Carries out the nuts and bolts of the seed to backbone analysis using network permutation.
%Implementation: See main paper and supplemental materials.
%Keywords: network permutation seed backbone
...
function [ total_nodes , numeric_dir_network , numeric_indir_network , numeric_context_network , observed , expected , p_values , p_values_for_seed_bb ,...
    ...
    np_seed2back_aux2( numeric_edgelist , numeric_seed_bb_list , numeric_bb_list , numeric_seed_list , perms , num_switches , protect_perm_step , parallelise , varargin )
...
not_seed_bb_indices = setdiff( node_indices , seed_bb_indices );

[ CC_mean_connectivity , seed_bb_indir_edge_counts , seed_bb_indir_edge_sum_weights ] = get_indirect_network_stats( network_matrix , seed_bb_indices , not_seed_bb_indices  );
...
all_perm_seed_bb_edge_counts = zeros( length( seed_bb_indices ) , perms );
all_perm_seed_bb_indir_edge_counts = zeros( length( seed_bb_indices ) , perms );
all_perm_seed_bb_indir_edge_sum_weights = zeros( length( seed_bb_indices ) , perms );
all_perm_mean_CC_connectivity = zeros( 1 , perms );

all_perm_num_edges_to_bb = zeros( 1 , perms );
    ...
    for I = 1:10:perms
        ...
        if( parallelise )
            parfor i = I:( min( [ ( I + 9 ) perms ] ) )
                drawnow % we can call drawnow to update gui even though we can't access gui_handles from within parfor loop... but we can't check for cancel
                ...
                all_perm_seed_bb_edge_counts( : , i ) = sum( network_matrix_permuted( seed_bb_indices , seed_bb_indices ) , 2 );
                
                [ perm_CC_mean_connectivity , perm_seed_bb_indir_edge_counts , perm_seed_bb_indir_edge_sum_weights ] =...
                    get_indirect_network_stats( network_matrix_permuted , seed_bb_indices , not_seed_bb_indices );
                
                all_perm_mean_CC_connectivity( i ) = perm_CC_mean_connectivity;
                all_perm_seed_bb_indir_edge_counts( : , i ) = perm_seed_bb_indir_edge_counts;
                all_perm_seed_bb_indir_edge_sum_weights( : , i ) = perm_seed_bb_indir_edge_sum_weights;
                
                all_perm_num_edges_to_bb( i ) = sum( sum( network_matrix_permuted( seed_indices , bb_indices ) ) );
            end
        else
...
observed_edge_count = sum( seed_bb_edge_counts )/2;

if( sum( seed_bb_edge_counts ) == 0 ) % find mean edge count
    observed_mean_edge_count = 0; % mean edge count is zero if there are no seeds in the network
else
    observed_mean_edge_count = mean( seed_bb_edge_counts( find( seed_bb_edge_counts ) ) ); % find mean edge count, but only for nodes in the network
end

if( sum( seed_bb_indir_edge_counts ) == 0 ) % find mean indirect edge count
    observed_mean_indir_edge_count = 0;
else
    observed_mean_indir_edge_count = mean( seed_bb_indir_edge_counts( find( seed_bb_indir_edge_counts ) ) );
end

if( sum( seed_bb_indir_edge_sum_weights ) == 0 ) % find mean indirect edge sum weight
    observed_mean_indir_edge_sum_weight = 0;
else
    observed_mean_indir_edge_sum_weight = mean( seed_bb_indir_edge_sum_weights( find( seed_bb_indir_edge_sum_weights ) ) );
end

observed_CC_mean_connectivity = CC_mean_connectivity;

expected_edge_count = mean( sum( all_perm_seed_bb_edge_counts , 1 ) / 2 );
p_value = ( length( find( ( sum( all_perm_seed_bb_edge_counts , 1 ) / 2 ) >= observed_edge_count ) ) + 1) / ( perms + 1 );

edge_count_means = sum( all_perm_seed_bb_edge_counts , 1 ) ./ sum( ( all_perm_seed_bb_edge_counts > 0 ) , 1 );
...
indirect_edge_count_means = sum( all_perm_seed_bb_indir_edge_counts , 1 ) ./ sum( ( all_perm_seed_bb_indir_edge_counts > 0 ) , 1 );
...
indirect_edge_sum_weight_means = sum( all_perm_seed_bb_indir_edge_sum_weights , 1 ) ./ sum( ( all_perm_seed_bb_indir_edge_sum_weights > 0 ) , 1 );
...
p_values_for_seed_bb = ( sum( bsxfun( @ge , all_perm_seed_bb_edge_counts , seed_bb_edge_counts ) , 2 ) + 1 ) ./ ( perms + 1 );
%****************************************************************************************************
...
num_edges_to_bb_exp = mean( all_perm_num_edges_to_bb );

% get the p_value for the number of edges to the backbone *******************************************
p_value_to_bb = ( length( find( all_perm_num_edges_to_bb >= num_edges_to_bb) ) + 1 ) / ( perms + 1 );
...
numeric_indir_network = get_numeric_indirect_network( network_matrix , seed_bb_indices , not_seed_bb_indices , ordered_nodelist );
end
