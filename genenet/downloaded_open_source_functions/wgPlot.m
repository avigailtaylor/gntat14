%Author: Michael Wu ( and minor changes by Avigail Taylor )
%Version: --
%Date: --
%Description: Plot the network.
%Implementation: DOWNLOADED FROM MATLAB CENTRAL. Minor changes by Avigail Taylor to alter display.
%Keywords: plot network
...
%   'vertexlabel' = N x 1 cell array of labels for nodes -- ADDED by A. Taylor.
% OUTPUT:
...
gui_handles_available=false; % added by A. Taylor
...
for kk=1:2:nVarArgin
    switch lower(varargin{kk})
        ...
        case 'vertexlabel' % added by A. Taylor
            vrtxLab=varargin{kk+1};
        case 'highlight' % added by A. Taylor
            highlight_indices=varargin{kk+1};            
        case 'gui_handles' % added by A. Taylor
...
if( gui_handles_available ) % Added by A. Taylor
  h=gui_handles.axes_quickview;
else
  h=gca;
end

% Set default parameter values % Re-arranged by A. Taylor
%--------------------
prh; 
hold on; axis off;
axesArea(h,[6 7 5 5]);
plotParm={'markerSize',6,'lineWidth',0.1,'marker','.','MarkerEdgeColor','yellow'};
siz=size(adjMat);
vrtxSiz=100;
edgeMap=cool;
vrtxMap=summer;
...
if exist('vrtxWt','var')
  vWt=vrtxWt;
  vWeighted = 1; % Added by A. Taylor to avoid spurious bug.
else
  vWt=diag(adjMat);
  vWeighted=length(setdiff(unique(vWt),0))>1; % A. Taylor moved this from after if statement to avoid spurious bug.
end
...
if eWeighted
  ...
  text([legLim(1),legLim(end)],[legY-0.5*yStep,legY-0.5*yStep], ...
    ...
    hE=plot(x(:),y(:),plotParm{:},'HandleVisibility', 'on');
end  % if eWeighted


% Plot vertices
%--------------------
if ( vWeighted & ( min(vWt) ~= max(vWt) ) ) % altered by A. Taylor to avoid bug
    ...
    if exist('vrtxCol','var')
        ...
        colorbar('HandleVisibility', 'on');
    else
        hV=scatter(coord(qq,1),coord(qq,2),vWt,'filled','MarkerFaceColor',[1,0.5,0.2]);      
    end
    hV=scatter(coord(qq,1),coord(qq,2),vWt,'filled','MarkerFaceColor','yellow'); %Added by A. Taylor - over-rides whatever happened in if statement.
else
    %hV=plot(coord(:,1),coord(:,2),plotParm{:},'LineStyle','none');
    hV=plot(coord(qq,1),coord(qq,2),plotParm{:},'LineStyle','none');
end

% Plot vertex labels % added by A. Taylor
if exist('vrtxLab','var')
  text(coord(:,1), coord(:,2), vrtxLab , 'VerticalAlignment','bottom', ...
                             'HorizontalAlignment','right' , 'Clipping' , 'on' , 'FontWeight' , 'bold' );
end

if exist('highlight_indices','var') % added by A. Taylor
  text(coord(highlight_indices,1), coord(highlight_indices,2), vrtxLab(highlight_indices) , 'VerticalAlignment','bottom', ...
                             'HorizontalAlignment','right' , 'Clipping' , 'on' , 'FontWeight' , 'bold' , 'Color' , 'magenta' );
end
...
if( gui_handles_available ) % added by A. Taylor
    ...
    axis tight;
    ax=axis;
    dxRange=1+(ax(2 )-ax(1 ) )/500; % added by A. Taylor
    dyRange=1+(ax(4 )-ax(3 ) )/500; % added by A. Taylor
    %axis tight;
    %ax=axis;
