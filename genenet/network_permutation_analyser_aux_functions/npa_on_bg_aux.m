...
%Version: 1
%Date: 20140722
%Description: Returns the network matrix corresponding to ntwk_edgelist_file with only the specified background nodes included, and analyses the background network (for the Network Permutation Analyser) as a side effect.
%Implementation: 
%Keywords: network background analysis nodes edges coefficient clustering assortativity neighbourhood metrics 
...
function network_matrix = npa_on_bg_aux( ntwk_edgelist_file , ntwk_header , bg_file , bg_header , gui_handles )

set( gui_handles.static_progress_status , 'string' , 'Loading background file...' );
drawnow
...
% read in the background nodelist file **************************************************************
bg_names_CELLARRAY = read_nodelist_file( bg_file , bg_header );
% ************************************************************************************************

% make sure the CELLARRAY lists each node name only once *****************************************
bg_names_CELLARRAY{1} = unique( bg_names_CELLARRAY{1} );


set( gui_handles.static_progress_status , 'string' , 'Making the background network edge list...' );
drawnow
if( get( gui_handles.toggle_cancel , 'Value' ) == 1 )
    err = MException( '' , 'User requested cancel');
    throw( err );
...
% get the ordered node list for the background network ******
[ ~ , numeric_edgelist ] = edgelist_file_2_ordered_nodelist_and_numeric_edgelist_inc_bg( ntwk_edgelist_file , ntwk_header , bg_names_CELLARRAY );
