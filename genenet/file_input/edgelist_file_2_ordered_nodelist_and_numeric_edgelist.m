...
%Version: 1
%Date: 20140722
%Description: Auxiliary function file. Turns an edgelist (stored as an edgelist file) in which the node names can have any format, into a list of edges where each of the nodes is replaced by a number. Also outputs the corresponding list of ordered nodes. Uses a perl script at the moment... may change to MATLAB in a later version.
%Implementation:
%Keywords: edge list names nodes ordered number auxiliary perl
...
function [ ordered_nodelist , numeric_edgelist ] = edgelist_file_2_ordered_nodelist_and_numeric_edgelist( ntwk_edgelist_file , header )
...
status_code = str2num( perl( 'edgelist_file_2_ordered_nodelist_and_numeric_edgelist_files.pl' , ntwk_edgelist_file , num2str( header ) , [ working_dir_str '/onf' ] , [ working_dir_str '/nef' ] ) );
...
switch status_code
    ...
    case -2
        rmdir( working_dir_str , 's' );
        err = MException( '' , 'Unable to open file in working folder (directory). Please check permissions.' );
        throw( err );
    case -1
        rmdir( working_dir_str , 's' );
